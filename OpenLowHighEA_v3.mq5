//+------------------------------------------------------------------+
//|                                       OpenLowHighEA_v3.mq5       |
//|                        Open=Low/High Strategy with Trailing SL   |
//|                        FIXED: Invalid Stops & Lot Calculation    |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   Print("=== OpenLowHigh EA v3.0 Initialized ===");
...
void OnDeinit(const int reason)
{
   ...
   Print("=== OpenLowHigh EA v3.0 Deinitialized ===");
}
...
void OnTick()
{
   ...
   {
      //--- Even if no new bar, manage trailing SL and dynamic SL
      ManageTrailingSL();
      if(InpUseDynamicSL)
         ManageDynamicSL();
      return;
   ...
   //--- Manage trailing for any open positions
   ManageTrailingSL();
   if(InpUseDynamicSL)
      ManageDynamicSL();
}
...
void ManageTrailingSL()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double thresholdUSD = InpMaxProfitUSD * (InpTrailingThreshold / 100.0);

      //--- Check if profit has reached threshold
      if(currentProfit >= thresholdUSD)
      {
         double newSL = 0;
         bool modifySL = false;
         ...
         if(posType == POSITION_TYPE_BUY)
         {
            //--- Phase 1: Move to breakeven if not already
            if(currentSL < openPrice)
            {
               newSL = openPrice;
               modifySL = true;
            }
            //--- Phase 2: Trail if profit exceeds threshold
            else if(currentProfit > thresholdUSD)
            {
