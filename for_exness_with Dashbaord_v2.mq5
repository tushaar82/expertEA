...
//| Init                                                             |
//+------------------------------------------------------------------+
//--- Chart Display Globals
double                  m_maxEquity      = 0.0;
double                  m_maxDrawdown    = 0.0;

//+------------------------------------------------------------------+
//| Update chart display - Profit & Drawdown                         |
//+------------------------------------------------------------------+
void UpdateChartDisplay()
{
   ...
   if(equity > m_maxEquity) m_maxEquity = equity;
   double drawdown = m_maxEquity - equity;
   if(drawdown > m_maxDrawdown) m_maxDrawdown = drawdown;

   // Format strings
   string profitStr = StringFormat("PROFIT: $%.2f", profit);
   string ddStr = StringFormat("DRAWDOWN: $%.2f (Max: $%.2f)", drawdown, m_maxDrawdown);
   string equityStr = StringFormat("EQUITY: $%.2f", equity);

   // Color: Green if profit > 0, Red if < 0, Gray if 0
   color profitColor = (profit > 0) ? clrLime : (profit < 0) ? clrRed : clrGray;
   color ddColor = (drawdown > 100) ? clrRed : clrYellow;

   // Create or update labels
   if(ObjectFind(0, "OHL_Profit") < 0)
   {
      ObjectCreate(0, "OHL_Profit", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, "OHL_Profit", OBJPROP_CORNER, CORNER_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_Profit", OBJPROP_ANCHOR, ANCHOR_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_Profit", OBJPROP_XDISTANCE, 10);
      ObjectSetInteger(0, "OHL_Profit", OBJPROP_YDISTANCE, 30);
      ObjectSetInteger(0, "OHL_Profit", OBJPROP_FONTSIZE, 16);
   }
   ObjectSetString(0, "OHL_Profit", OBJPROP_TEXT, profitStr);
   ObjectSetInteger(0, "OHL_Profit", OBJPROP_COLOR, profitColor);

   if(ObjectFind(0, "OHL_DD") < 0)
   {
      ObjectCreate(0, "OHL_DD", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, "OHL_DD", OBJPROP_CORNER, CORNER_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_DD", OBJPROP_ANCHOR, ANCHOR_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_DD", OBJPROP_XDISTANCE, 10);
      ObjectSetInteger(0, "OHL_DD", OBJPROP_YDISTANCE, 55);
      ObjectSetInteger(0, "OHL_DD", OBJPROP_FONTSIZE, 12);
   }
   ObjectSetString(0, "OHL_DD", OBJPROP_TEXT, ddStr);
   ObjectSetInteger(0, "OHL_DD", OBJPROP_COLOR, ddColor);

   if(ObjectFind(0, "OHL_Equity") < 0)
   {
      ObjectCreate(0, "OHL_Equity", OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, "OHL_Equity", OBJPROP_CORNER, CORNER_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_Equity", OBJPROP_ANCHOR, ANCHOR_RIGHT_UPPER);
      ObjectSetInteger(0, "OHL_Equity", OBJPROP_XDISTANCE, 10);
      ObjectSetInteger(0, "OHL_Equity", OBJPROP_YDISTANCE, 75);
      ObjectSetInteger(0, "OHL_Equity", OBJPROP_FONTSIZE, 10);
   }
   ObjectSetString(0, "OHL_Equity", OBJPROP_TEXT, equityStr);
   ObjectSetInteger(0, "OHL_Equity", OBJPROP_COLOR, clrWhite);
...
void Order(ENUM_ORDER_TYPE type, double price, double lot)
{
   ...
   if(result)
   {
      ...
      m_orderCount++;
      Print((type == ORDER_TYPE_BUY ? "BUY" : "SELL"), " ", price, " Lot:", lot);
   }
   else
   {
